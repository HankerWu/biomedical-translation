...
class PDF_Content:
    ...
    def getContent(self):
        def title_test():
            if len(self.title) < 10:  #总长过短
                return False
            # getEndIndex=re.compile(r'[0-9 \.]*(.*)')
            getEnd = re.compile(r'[A-Za-z]+')
            # title_az=getEndIndex.match(self.title).group(1)
            matchEnd = getEnd.findall(self.title)
            if len(matchEnd) < 3:  #单词数过短
                return False
            return True

        self.__getTitleByPyPDF2()  #使用PyPDF2获得标题
        if title_test() is not True:
            self.title = ''
            self.__getTitleByPdfplumber()  #使用Pdfplumber获得标题
    ...
    def __getTitleByPdfplumber(self):
        with pdfplumber.open(self.path) as pdf:
            ...
            for i in range(len(size)):
                max_size = size[-i - 1]
                for word in words:  #找到第一个这么大的词的位置
                    word_top = 0
                    if len(word['text']) < 5:  #过短
                        continue
                    if word['size'] == max_size:
                        word_top = word['top']
                        break
                for word in words:  #找到附近剩下的这么大的词
                    if word['size'] == max_size and word[
                            'top'] < word_top + 3 * max_size:
    ...
    def __extractText(self):
        def extract_text_within_bbox(page, box):
            return page.within_bbox(box).extract_text(x_tolerance=2,
                                                      y_tolerance=2)
        ...
        def get_main_body_row_height_and_font_size(pdf):
            ...
            return delta_y0_argmax, font_size_argmax
        ...
        def division_column(page, width, bbox_top, bbox_bottom,
                            main_font_size):
            words = page.within_bbox([0, bbox_top, width, bbox_bottom
                                      ]).extract_words(x_tolerance=2,
                                                       y_tolerance=2,
                                                       keep_blank_chars=True,
                                                       use_text_flow=True)
            #统计单词开头的x位置
            ...
            font_size_argmax = np.argmax(font_size_counts)
            #字号过小
            if font_size_argmax < Decimal('0.90') * main_font_size:
                return
            #寻找 最多和次多 单词开头 的x位置
            ...
            if x0_counts[x0_second] > 10:
                ...
                if x0_second - x0_first > width * Decimal('0.2'):
                    temp = extract_text_within_bbox(
                        page, [x0_first, bbox_top, x0_second, bbox_bottom])
                    if temp is not None:
                ...
                if width - x0_second > width * Decimal('0.2'):
                    temp = extract_text_within_bbox(
                        page, [x0_second, bbox_top, width, bbox_bottom])
                    if temp is not None:
                        self.__extract_text += temp + '\n'
                        #print(temp+'\n')
            else:
                #否则认为是单栏结构
                temp = extract_text_within_bbox(
                    page, [x0_first, bbox_top, width, bbox_bottom])
                if temp is not None:
                    self.__extract_text += temp + '\n'
                    #print(temp+'\n')
        ...
        with pdfplumber.open(self.path) as pdf:
            ...
            for i in range(len(pdf.pages)):
                ...
                #去除固定的页眉页脚
                header = Decimal('0.05') * pdf.pages[i].height
                footer = (Decimal('1') - Decimal('0.05')) * pdf.pages[i].height
                row_pos = np.append(row_pos, [header, footer])
                row_pos = row_pos[np.where((row_pos >= header)
                                           & (row_pos <= footer))]
                ...
                for j in range(1, len(row_pos)):
                    ...
                    division_column(pdf.pages[i], pdf.pages[i].width, row_top,
                                    row_buttom, main_font_size)
    ...
    def __getAbstractAndIntroduction(self):
        ...
        getEndIndex = re.compile(r'[0-9 \.]*(.*)')
        getEnd = re.compile(r'[A-Za-z]+')
        getPeriodEnd = re.compile(r'\.[\s0-9_\[\]]*$')  #句号结尾
        getBlank = re.compile(r'^[\s]*$')  #空的行
        ...
        last_line = []
        for line in lines:
            if len(getBlank.findall(line)) > 0:  #空行
                # last_line = line
                continue
            if (len(line) < 5):  #过于短的行
                continue
            ...
            if abs_flag == 1:
                ...
                matchPeriodEnd = getPeriodEnd.findall(last_line)  # 上一行以句号结尾
                if len(matchEnd) <= 3 and len(
                        matchEnd) > 0 and '.' not in line and len(
                            matchPeriodEnd) > 0:
                    abs_flag = 2
                else:
                    self.abstract += line + ' '
