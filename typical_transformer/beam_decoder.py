...
class Beam:
    ...
    def __init__(self, size, pad, bos, eos, device=False):
        ...
        self.next_ys = [
            torch.full((size, ), self.PAD, dtype=torch.long, device=device)
        ]
    ...
    def advance(self, word_logprob):
        ...
        if len(self.prev_ks) > 0:
            beam_lk = word_logprob + self.scores.unsqueeze(1).expand_as(
                word_logprob)
        else:
            # in initial case,
...
def beam_search(model, src, src_mask, max_len, pad, bos, eos, beam_size,
                device):
    ...
    def collate_active_info(src_enc, src_mask, inst_idx_to_position_map,
        ...
        active_src_enc = collect_active_part(src_enc, active_inst_idx,
                                             n_prev_active_inst, beam_size)
        active_inst_idx_to_position_map = get_inst_idx_to_tensor_position_map(
            active_inst_idx_list)
        active_src_mask = collect_active_part(src_mask, active_inst_idx,
                                              n_prev_active_inst, beam_size)
    ...
    def beam_decode_step(inst_dec_beams, len_dec_seq, enc_output,
        ...
        def predict_word(dec_seq, enc_output, n_active_inst, n_bm):
            ...
            out = model.decode(
                enc_output, src_mask, dec_seq,
                subsequent_mask(dec_seq.size(1)).type_as(src.data))
        ...
        def collect_active_inst_idx_list(inst_beams, word_prob,
            ...
            for inst_idx, inst_position in inst_idx_to_position_map.items():
                ...
                )  # Fill Beam object with assigned probabilities
    ...
    with torch.no_grad():
        ...
        src_enc = src_enc.repeat(1, beam_size, 1).view(batch_size * beam_size,
                                                       sent_len, h_dim)
        src_mask = src_mask.repeat(1, beam_size,
                                   1).view(batch_size * beam_size, 1,
                                           src_mask.shape[-1])

        # -- Prepare beams
        inst_dec_beams = [
            Beam(beam_size, pad, bos, eos, device) for _ in range(batch_size)
        ]
        ...
        for len_dec_seq in range(1, max_len + 1):
            ...
            src_enc, src_mask, inst_idx_to_position_map = collate_active_info(
                src_enc, src_mask, inst_idx_to_position_map,
                active_inst_idx_list)
